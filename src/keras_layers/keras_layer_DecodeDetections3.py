...
class DecodeDetections(Layer):
    ...
    def call(self, y_pred, mask=None):
        ...
        def filter_predictions(batch_item):
            ...
            def filter_single_class(index):
                ...
                def perform_nms():
                    ...
                    maxima_indices = tf.image.non_max_suppression(boxes=boxes,
                                                                  scores=scores,
                                                                  score_threshold=self.tf_confidence_thresh,
                ...
                padded_single_class = tf.pad(tensor=single_class_nms,
                                             paddings=[[0, tf.maximum(self.tf_nms_max_output_size - tf.shape(single_class_nms)[0], 0)], [0, 0]],
                                             mode='CONSTANT',
                                             constant_values=0.0)
            ...
            filtered_single_classes = []
            for i in range(1,n_classes):
              filtered_single_classes.append(tf.cast(filter_single_class(i), tf.float32))
            filtered_single_classes = tf.convert_to_tensor(filtered_single_classes)
            ...
            def pad_and_top_k():
                padded_predictions = tf.pad(tensor=filtered_predictions,
                                            paddings=[[0, tf.maximum(self.tf_top_k - tf.shape(filtered_predictions)[0],0)], [0, 0]],
                                            mode='CONSTANT',
                                            constant_values=0.0)
        ...
        # output_tensor=[]
        # for i in range(int(tf.shape(y_pred)[0])):
        #     output_tensor.append(filter_predictions(y_pred[i]))
        # tf.convert_to_tensor(output_tensor)


        # output_tensor = filter_predictions(y_pred[0])
